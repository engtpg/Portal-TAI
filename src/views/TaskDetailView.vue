...
onMounted(async () => {
  ...
  onSnapshot(taskDocRef, (doc) => {
    if (doc.exists()) {
      task.value = { ...doc.data(), docId: doc.id }
  ...
  const actionsQuery = query(
    collection(db, 'tasks', taskId, 'actions'),
...
const selectUserForMention = (username) => {
  ...
  showMentionList.value = false
  document.getElementById('newActionTask').focus()
}
...
const addAction = async () => {
  ...
  try {
      ...
      for (const mention of uniqueMentions) {
        ...
        if (mentionedItem && mentionedItem.type === 'user' && mentionedItem.uid !== user.uid) {
          ...
          await addDoc(notificationRef, {
            message: `${user.displayName} mention Anda di task: "${task.value.taskName}"`,
            link: `/tasks/${taskId}`,
            isRead: false,
          ...
          for (const member of mentionedItem.members) {
            if (member.uid !== user.uid) {
              ...
              await addDoc(notificationRef, {
                message: `${user.displayName} mention grup Anda (@${mentionedItem.username}) di task: "${task.value.taskName}"`,
                link: `/tasks/${taskId}`,
                isRead: false,
                createdAt: serverTimestamp(),
              })
    ...
    await addDoc(collection(taskDocRef, 'actions'), {
...
*Nama Task* : ${task.value.taskName}

*Dibuat* : ${task.value.creatorName}
*Start* : ${formatDate(task.value.createdAt)}
*Closed* : ${task.value.closedAt ? formatDate(task.value.closedAt) : 'N/A'}

*Last Action* : ${task.value.lastAction}
...
<template>
  <div v-if="loading" class="text-center">Memuat detail task...</div>
  <div v-else-if="task" class="grid grid-cols-1 lg:grid-cols-3 lg:gap-8">
    <!-- Kolom Kiri (Konten Utama) -->
    <div class="lg:col-span-2 space-y-8">
      <!-- Header -->
      <div>
        <nav class="text-sm font-medium text-gray-500 dark:text-gray-400 mb-2">
          <RouterLink to="/tasks" class="hover:underline">Daftar Task</RouterLink>
          <span class="mx-2">/</span>
          <span class="text-gray-700 dark:text-gray-200">Detail</span>
        </nav>
        <h1 class="text-3xl font-bold text-gray-800 dark:text-white">{{ task.taskName }}</h1>
        <p class="text-sm text-gray-500 mt-1">
          ID: {{ task.id }} &bull; Dibuat oleh
          <RouterLink
            :to="`/profile/${task.creatorUid}`"
      ...
      <div class="grid grid-cols-1 md:grid-cols-3 gap-6">
        <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md">
          ...
          <p class="text-lg font-bold text-gray-800 dark:text-white">{{ task.status }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md">
          <p class="text-sm text-gray-500">Priority</p>
          <p class="text-lg font-bold text-gray-800 dark:text-white">{{ task.priority }}</p>
        </div>
        <div class="bg-white dark:bg-gray-800 p-5 rounded-xl shadow-md">
        ...
        <form v-else @submit.prevent="addAction" class="space-y-4 max-w-3xl mx-auto">
          ...
          <div class="flex justify-end space-x-4 pt-4">
            ...
            <button
              ...
              class="px-6 py-2 font-medium text-white bg-orange-500 rounded-lg hover:bg-orange-600 disabled:bg-orange-400"
            >
    ...
    <!-- Kolom Kanan (Salin Data) (disembunyikan untuk guest) -->
  ...
  <div v-else><h1 class="text-center text-2xl">Task tidak ditemukan.</h1></div>
</template>
